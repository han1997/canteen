...
- 管理员在"菜品管理"中**首次**创建某天某餐次的 slot 时，后端会自动从「同餐次、日期严格早于本日」的最近一次 slot 复制全部未删除菜品（含图片/价格/营养字段/MealPackageItem），免去逐项重录；如已存在 slot 或已有菜品，不会重复克隆。
- 菜品支持软删除（`meal_packages.is_deleted`）：删除后用户端/统计/下单均不可见，但历史订单仍可正常导出与核销。
